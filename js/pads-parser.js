...
(function (root, factory) {
  ...
  PADSParser.prototype._parttypeLine = function (line) {
    ...
    var name = tokens[0];
    var decal = tokens[1];
    if (name.charAt(0) === '"' || decal.charAt(0) === '"') return;
    // Type names may start with a digit (e.g. "0402CS") but are never
    // purely numeric — that filters pin/coordinate rows.
    if (/^-?[\d.]+$/.test(name)) return;
    // Decals CAN be purely numeric (chip sizes: "0603", "0805", "1812").
    // What distinguishes a real item row from gate rows like "G 1 2" or
    // "GATE 1 48 0" is that item rows carry a family name and flag letters
    // after the decal, while gate rows are numeric all the way.
    if (/^-?[\d.]+$/.test(decal)) {
      var allNumeric = true;
    ...
    if (this._ptypeDecals[name] === undefined) {
      this._ptypeDecals[name] = decal.split(":")[0];
    }
  };
